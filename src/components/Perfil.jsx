...
const Perfil = ({ usuario, user, setData, data, setFlagUsuario }) => {
  ...
  const handleDelete = (user) => {
    swal({
      title: "Eliminar",
      text: "¿Estás seguro(a) de eliminar tu perfil?",
      icon: "warning",
      buttons: ["No", "Si"],
    }).then((respuesta) => {
      if (respuesta) {
        ...
        swal({ text: "El archivo se elimino correctamente!", icon: "success" });
  ...
  const handleUpdate = (e) => {
    ...
    swal({
      ...
      text: "¿Estás seguro(a) de editar tu perfil?",
      icon: "warning",
      buttons: ["No", "Si"],
    }).then((respuesta) => {
      if (respuesta) {
        ...
        swal({ text: "Información actualizada correctamente!", icon: "success" });
        document.getElementById("oculto").click();
      }
    });
  };
  ...
  const handleCloseSesion = () => {
    setFlagUsuario(false);
    swal({title:"Se ha cerrado sesión correctamente!",
          icon:"success"})
    document.getElementById("oculto").click();
  };
  ...
  return (
    <div>
      <div className={styles.container}>
        <button onClick={handleCloseSesion} className={styles.buttonCerrar}>
          Cerrar Sesión
        </button>
        <div>
          <form className={styles.form_login}>
            <label for="user">Nombre de Usuario</label>
            <input id="user" type="text" value={user} disabled />
            ...
            {banderaUpdate ? (
              ...
              <button hidden type="button" className={styles.button_guardar}>
                Guardar Cambios
              </button>
            )}
            <button
